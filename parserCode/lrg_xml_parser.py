...
#model the local workspace - execute from parent directory if none is provided in main statement
    #workspace diagram:
...
class Workspace(object):
    ...
    #can specify parent directory if requried  	
...
class Test_xml_parser(object):

   def test_exon_coordinates(self):
        WS = Workspace()
        lrg1_test_instance = Lrg(WS.root_dir + '/lrgs/LRG_1/LRG_1.xml')
        exon_coords = lrg1_test_instance.get_exon_coords()
        assert (lrg1_test_instance.id == 'LRG_1'), "LRG_ID incorrect, ensure LRG1.xml is in the correct directory. see readme for directory structure"
        assert (len(exon_coords) == 51), "the incorrect number of cordinates from LRG1 has been read. The method LRG.get_exon_coords() has failed."
        exon_1 = exon_coords["1"]
        assert (exon_1[0] == "5001"), "start coordinates for the first exon are not as expected. The method LRG.get_exon_coords() has failed."
        assert (exon_1[1] == "5229"), "end coordinates for the first exon are not as expected. The method LRG.get_exon_coords() has failed."

#main statment to initialise lrg instances in an array and loop through each to perform functions
if __name__ == "__main__":
    Test_xml_parser().test_exon_coordinates()
    WS = Workspace()
    lrg_instances = WS.lrg_array()
    for lrg in lrg_instances:
	name = lrg.get_name()
	format = lrg.get_format()
        if format == 'xml':
	    lrg.get_id()
	    print lrg.get_exon_coords()
	    lrg.get_intron_coords()
	    print lrg.get_sequences()
        else:
            print 'lrg_format is ' + format + ' for ' + name + '. xml format is required'
